...
function SearchForm({ onSearchButtonClick, inputMovieName, defaultValue }) {
  ...
  useEffect(() => {    
    updateShortFilmSaved(JSON.parse(localStorage.getItem("shortFilmSaved" || "false")));
  }, []);
...
function SearchForm({ onSearchButtonClick, inputMovieName, defaultValue }) {
  ...
  function handleChangeCheckButtonSaved(evt) {
    const switchShortFilmSaved = evt.target.checked;
    updateShortFilm(switchShortFilmSaved);
    onSearchButtonClick(movieName, switchShortFilmSaved);
  }
